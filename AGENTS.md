...
6. **Optional hard gate (Cursor hooks)** — Rules alone cannot stop **`Write`** / edit tools. For stricter enforcement, copy the **preToolUse** template from **`vendor/warpdesk-client-kit/templates/cursor-hooks/`** into **`.cursor/hooks.json`** (see **`README.md`** there): it reads **`.warpdesk/clock-local-state.json`** and can **`deny`** or **`ask`** when **`phase`** is not **`dev`** (optional env to allow **`cursor`**). Requires **Node on PATH**; paths under **`.warpdesk/`**, **`vendor/`**, and **`knowledge/`** stay allowed so config and drafts still work.
