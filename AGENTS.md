...
| Goal | Steps |
| ---- | ----- |
| **Fetch ticket data** | MCP **`list_priority_active_tickets`** (optional `band`), **`get_ticket`**, **`get_ticket_by_number`**, **`search_tickets`** — each success refreshes **`.warpdesk/tickets.ticketselector`**. Or **`npm run warpdesk:tickets`** / CLI **`list`**. Open **`.warpdesk/tickets.ticketselector`** in WarpDesk Tools. |
